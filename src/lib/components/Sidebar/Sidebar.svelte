<script lang="ts">
  ...
  function autoBetDropBall() {
    ...
    if (autoBetsLeft === null) {
      $plinkoEngine?.dropBall(
        $isPreselectEnabled && $preselectedBinIndex !== null ? $preselectedBinIndex : undefined,
      );
      return;
    }
  ...
  const binOptions = $derived(
    Array.from({ length: $rowCount + 1 }, (_, i) => ({ value: i, label: i.toString() })),
  );
</script>
...
<div class="flex flex-col gap-5 bg-slate-700 p-3 lg:max-w-80">
  ...
  <div>
    <div class="flex items-center justify-between">
      <label for="preselectToggle" class="text-sm font-medium text-slate-300">Preselect Bin</label>
      <Switch
        id="preselectToggle"
        bind:checked={$isPreselectEnabled}
        disabled={hasOutstandingBalls || autoBetInterval !== null}
      />
    </div>
  </div>
  ...
  <div>
    <label for="targetBin" class="text-sm font-medium text-slate-300">Target Bin</label>
    <Select
      id="targetBin"
      bind:value={$preselectedBinIndex}
      items={binOptions}
      disabled={!$isPreselectEnabled || hasOutstandingBalls || autoBetInterval !== null}
    />
  </div>
