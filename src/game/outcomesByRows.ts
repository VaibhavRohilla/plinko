...
// Convert legacy pixel-based outcomes (padded px) into normalized slot offsets for rows=18
function legacyToOffsets(): number[][] {
  const keys = Object.keys(outcomes).map((k) => Number(k)).sort((a, b) => a - b);
  const byIndex: number[][] = [];
  for (const k of keys) {
    const arr = (outcomes as any)[String(k)] as number[] | undefined;
    if (!arr || arr.length === 0) {
      byIndex[k] = [];
      continue;
    }
...
// Seed with transformed legacy data for 18 rows; add more rows via calibration
